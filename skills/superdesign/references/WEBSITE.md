...
- **Borrow a site's style** (e.g. "design … in the style of linear.app"): `extract-website --url <site> --design-md` → read `design.md` (a portable style guide) and fold it into `.superdesign/design-system.md` (SOP: BRAND NEW PROJECT Step 2 in [SUPERDESIGN.md](SUPERDESIGN.md) — choose **create-from / inspired-by / update-existing**; if a `design-system.md` already exists, ASK before overwriting), then design as usual. Add `--brand` for logo/colors/fonts (`brand.json`). Upload only the approved reusable identity files with `--purpose brand --type <logo|font|image> --key <stable-key> --description <use>`; pass the returned Brand Asset key via `--reference-id` when its pixels must guide generation.
